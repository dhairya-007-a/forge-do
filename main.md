...
- `ADMIN_API_KEY` is still necessarily present in `admin-dashboard.html`'s plain JS (needed so the page's own fetch calls can auto-send it) — same threat model as any client-embedded secret on a static site. Acceptable for now given this is a small personal/student project, not handling sensitive data at scale. **The Basic-Auth password is no longer duplicated in JS** (fixed 2026-08-09) — the browser caches it per origin+realm after admin-gate.js's first challenge and auto-resends it to same-realm fetch() calls, so it only needs to exist once, server-side.
